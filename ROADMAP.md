...
0. **Arrancar la verificación del negocio en Meta** (primer paso de **BL-15**, Fase 4): es lo único que no depende de nosotros y puede tardar días, así que conviene largarlo antes que nada y seguir con el resto mientras tanto. BL-15 es lo que hace que *cualquier* visitante de la demo pueda chatear con el agente, sin cargar su número a mano en Meta.
...
- [ ] **BL-15 — Abrir el agente de WhatsApp a cualquier visitante de la demo** (hoy solo pueden hablarle ~5 números cargados a mano en Meta). Es el mismo trámite que BL-03 pero **para nuestro número de demo, no el de un cliente** — no confundirlos. Un prospecto que entra a `demo.fersiontech.com`, toca el botón y escribe, hoy **no recibe respuesta** (error 131030). Pasos, en este orden:
  1. **Arrancar ya la verificación del negocio en Meta** (app `fersion` → WhatsApp → Configuración básica → Paso 3). Gratis, pero puede tardar días y no depende de nosotros — es lo único bloqueante por tiempo.
  2. **Conseguir un número dedicado** (una SIM prepaga alcanza). ⚠️ El número que se registra en la Cloud API **no puede tener una cuenta de WhatsApp activa** (ni personal ni Business App): al registrarlo se pierde WhatsApp normal en ese número. **No usar el número personal acá.** Son dos slots distintos: `WHATSAPP_PHONE_NUMBER_ID`/`WHATSAPP_CHAT_NUMBER` = el número con el que hablan los visitantes (dedicado); `WHATSAPP_AGENT_NUMBER` = dónde llegan los avisos de lead nuevo (ahí sí va el número personal).
