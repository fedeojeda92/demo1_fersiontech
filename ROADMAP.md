...
Sesión del 2026-09-11 (noche, continuación) cerrada con: BL-08 hecha (limpieza de leads de prueba) y BL-06 decidida (seguir en Vercel Hobby, upgradear a Pro recién cuando el primer cliente pague). Se chequeó BL-01 en el Administrador de WhatsApp de Meta: `nuevo_lead_v2` sigue **"En revisión"**, sin novedad. También se borró `tokenwspfersion.txt` (archivo suelto fuera del repo con un token de WhatsApp en texto plano, resabio de BL-02 — ya no hacía falta, el token vigente vive en `.env.local`/Vercel).
...
2. **BL-03 (número de WhatsApp Business real)** depende de que BL-01 esté resuelto — no arrancarlo antes.
3. Con las tareas chicas de Fase 1.B cerradas (salvo BL-01/BL-03 que dependen de Meta), lo más grande e importante que queda de Fase 1 es **1.A — el agente conversacional de WhatsApp con IA (RM-01 a RM-10)**, que todavía no se empezó. Es la pieza central que falta antes de poder vender.
