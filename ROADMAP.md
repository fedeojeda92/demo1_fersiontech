...
Sesión del 2026-09-23 (cierre): **la demo ya se puede mostrar sola.** Cualquier prospecto que entre a `demo.fersiontech.com` puede probar el agente de IA desde el chat de la web (BL-13), sin que haga falta el chip de WhatsApp ni cargar su número en Meta — eso destraba el problema de "no puedo mostrar el producto" sin gastar un peso. En el camino se arregló un bug que venía fallando en silencio en WhatsApp desde siempre (el filtro de zonas) y se recuperó el agente, que había dejado de responder por un 503 sostenido de Gemini. Lo que falta para salir a vender sigue siendo comercial/legal, no técnico.

**Próximos pasos, en orden:**
0. **Verificar el chat web en producción** (BL-13, hecho el 2026-09-23): abrir `demo.fersiontech.com`, probar el chat y confirmar que **el link de una propiedad sale con el dominio bueno y no con la URL de Vercel** (mismo chequeo que el punto 2 de abajo, `getSiteUrl` en `src/lib/agent/tools.ts`). Después, `delete from leads where source = 'web_demo';`.

   **BL-15 (el chip de WhatsApp) dejó de ser urgente**: el chat web ya permite que cualquier prospecto pruebe el agente sin gastar un peso ni cargar su número en Meta. El chip pasa a ser parte del onboarding del cliente que compra, igual que BL-03. Para una demo en vivo por WhatsApp con un prospecto ya interesado alcanzan los 5 destinatarios de prueba.
...
**Estado al 2026-09-23:** Fase 1 completa; Fase 4 con la demo lista y ahora **autodemostrable** (chat de IA abierto en la web, BL-13 — falta la sección de planes de fersiontech.com); de Fase 2 se adelantó BL-13; de Fase 5 está hecho el monotributo y el borrador del guión. **Lo que separa de salir a vender: revisar el guión del pitch, PDF one-pager y la sección de planes de la web.**
